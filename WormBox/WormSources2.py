...
class Neuron(BuiltinSurface):
    ...
    def __init__(self,center,*args,**kwargs):
        ...
        self.name='bis'
        self.source='sphere'
        self.data_source.center=self.center
...
class SphereMesh(PolarMesh,HasTraits):#Assumes all mesh patches to have the same shape
    ...
    def update_plot(self,offset=[0.0,0.0,0.0]):
...
#        self.do_iter()#without cython. Doesn't do eccentricity either
        ...
        o_mz,o_mx,o_my=self.apply_offset([mz,mx,my],offset) 
        self.small_mesh.mlab_source.set(x=o_mz,y=o_mx,z=o_my)
#        o_mz,o_mx,o_my=self.apply_offset([mz,mx,my],offset)        
...
#        return mz,mx,my
    def update_zeroed_plot(self,offset=[0.0,0.0,0.0]):
        offset=np.array(offset)
        for i in range(100):
            self.do_cython_iter()
        mz,mx,my=self.get_zeroed_mesh_coords()
        o_mz,o_mx,o_my=self.apply_offset([mz,mx,my],offset) 
        self.small_mesh.mlab_source.set(x=o_mz,y=o_mx,z=o_my)
...
#####DEBUG#####
    def update_mesh_coords(self):
        RadiusWrap=np.vstack([self.Radius,self.Radius[0,:][np.newaxis,:]])#Add theta at 2pi in addition to at 0 
        mz= self.smear[0]*RadiusWrap*np.cos(self.phi_mesh) + self.Center[0]                         *self.spacing[0]
        my= self.smear[1]*RadiusWrap*np.sin(self.phi_mesh)*np.sin(self.theta_mesh) + self.Center[2] *self.spacing[1]
        mx= self.smear[2]*RadiusWrap*np.sin(self.phi_mesh)*np.cos(self.theta_mesh) + self.Center[1] *self.spacing[2]
        return mz,mx,my
    def get_zeroed_mesh_coords(self):
        RadiusWrap=np.vstack([self.Radius,self.Radius[0,:][np.newaxis,:]])#Add theta at 2pi in addition to at 0 
        mz= self.smear[0]*RadiusWrap*np.cos(self.phi_mesh) + (self.Center[0]-self.c_est[0])                         *self.spacing[0]
        my= self.smear[1]*RadiusWrap*np.sin(self.phi_mesh)*np.sin(self.theta_mesh) + (self.Center[2]-self.c_est[1]) *self.spacing[1]
        mx= self.smear[2]*RadiusWrap*np.sin(self.phi_mesh)*np.cos(self.theta_mesh) + (self.Center[1]-self.c_est[2]) *self.spacing[2]
        return mz,mx,my
        
    def draw_mesh_on_small_scene(self,scene,offset=[0.0,0.0,0.0]):
        offset=np.array(offset)
        print 'WormSource2.py: draw_mesh_on_small_scene() fired'
#        mz,mx,my=self.update_mesh_coords()
        mz,mx,my=self.get_zeroed_mesh_coords()
        o_mz,o_my,o_mx=self.apply_offset([mz,my,mx],offset)
#        self.small_mesh=mlab.mesh(mz,mx,my,representation='wireframe',color=(1,1,1),figure=scene.mayavi_scene)
        self.small_mesh=mlab.mesh(o_mz,o_mx,o_my,representation='wireframe',color=(1,1,1),figure=scene.mayavi_scene)
        self.small_mesh.name='small sphere mesh'
...
##                            scale_factor=10)    
#    
#    from WormBox.WormSources2 import LocalWindow
#    
##    vis=Interactor(I)
#    vis=LocalWindow(I.shape,I)
