...
class SphereParams(object):
    ...
    theta_steps=40
    phi_steps=20
#    phi_steps=120#debug ellipsoidal

#        self.N=8 #number of nonzero frequencies
    N=5
#    N=10#Debug ellipsoidal
...
#        self.Wml=np.zeros(self.nF)


class Ellipse:
    def __init__(self,spacing,r0):
        self.spacing=spacing
        self.r0=r0

    def calc_radius(self,angle):
        theta,psi=angle
        x=np.cos(theta)*np.sin(psi)/self.spacing[0]
        y=np.sin(theta)*np.sin(psi)/self.spacing[1]
        z=np.cos(psi)/self.spacing[2]
        radius_multiplier=np.sqrt(x**2+y**2+z**2)**-1
#        radius_multiplier/=np.mean(radius_multiplier)
        return self.r0*radius_multiplier
            
            
            
            
def EllipseRadiusScale(angle,unscaled_spacing,r0):#spacing should be x,y,z order
#    alpha=np.sqrt( 1/(2*unscaled_spacing[0]) + 1/(2*unscaled_spacing[1]**2) )/r0
#    spacing=alpha*unscaled_spacing
    spacing=r0*unscaled_spacing
#    spacing=unscaled_spacing/np.linalg.norm(unscaled_spacing)
    theta,psi=angle
    x=np.cos(theta)*np.sin(psi)/spacing[0]
    y=np.sin(theta)*np.sin(psi)/spacing[1]
    z=np.cos(psi)/spacing[2]
    radius_multiplier=np.sqrt(x**2+y**2+z**2)**-1
    radius_multiplier/=np.mean(radius_multiplier)
...
def squish(psi,s):
    num=np.cos(psi)
    dem=2*np.cos( (np.pi/2)*(s-1)/s + psi/s )
    return num/dem

def Ys(freq,angle,spacing):#This function is defunct
    m,n=freq
    theta,psi=angle
    if m<0:
        Y = np.sqrt(2)*( -1)**m * np.imag(sph_harm(-m,n,theta,psi))
    elif m==0:
        Y = np.real(sph_harm(m,n,theta,psi))
    elif m>0:
        Y = np.sqrt(2)*( -1)**m * np.real(sph_harm(m,n,theta,psi))
    s=np.double(spacing[0])/spacing[1]
    if spacing[1]!=spacing[2]:print "warning x and y spacing different"
    return squish(psi,s)*Y
...
class ReferenceGrid():
    def __init__(self,Ishape,spacing,smear):
        self.spacing=spacing
#        self.I=I
        self.Ishape=Ishape
...
#        self.Distance=np.sqrt( (self.z*spacing[0])**2 + ((self.x)*spacing[1])**2  +  ((self.y)*spacing[2])**2  )
    ...
    ##DEBUG##
#        self.Distance=np.sqrt( (self.z*smear[0])**2 + ((self.x*smear[2]))**2  +  ((self.y*smear[1]))**2  )#smear is z,y,x
#        self.Distance=np.sqrt( (self.z/smear[0])**2 + ((self.x/smear[2]))**2  +  ((self.y/smear[1]))**2  )#smear is z,y,x
#        print 'Nucleus.py line234:'
#        print 'smear is list?'
#        print '    ',isinstance(smear,list)
#        print 'spacing is list?'
#        print '    ',isinstance(spacing,list)
        scale=smear/spacing
        self.Distance=np.sqrt( (self.z/scale[0])**2 + ((self.x/scale[2]))**2  +  ((self.y/scale[1]))**2  )#scale is z,y,x
...
class PolarCache(SphereParams):
    ...
    def __init__(self,patch_shape,spacing,smear):
        self.smear=smear
        self.spacing=spacing
...
#        self.Wml=np.ones(self.Aml.shape).astype(np.float)
        ...
        self.ref=ReferenceGrid(self.patch_shape,self.spacing,self.smear)  #Do this later. Hope radius doesn't hit patch boundary
        ...
        self.Awt=np.array([Y(w, Iangle ) for w in self.Freq])
        #EXPERIMENTAL! DEBUG
#        self.Awt=np.array([Ys(w, Iangle,self.spacing ) for w in self.Freq])#Rescale bases for spacing...    
#        self.Awt=np.array([Y(w, Iangle ) for w in self.Freq])
#        self.ERS=EllipseRadiusScale(Iangle,self.spacing[::-1],self.r0)
        #EXPERIMENTAL! DEBUG
...
#                print 'not a np array'
        ...
        cy_u1,cy_delA,cy_delc,cy_delr0=cy_iteration(
            self.spacing,self.Radius,self.I,self.std_size,self.ref.ref_size,self.ref.ref_shape.astype(np.int),
#            self.std_shape.astype(np.int),self.ref.Dcrt_Angle,self.ref.Distance,self.Awt,np.array(self.Awt.shape),self.ERS,
            self.std_shape.astype(np.int),self.ref.Dcrt_Angle,self.ref.Distance,self.Awt,np.array(self.Awt.shape),
            self.Aml,self.Wml,self.Center,self.c_est,self.r0,self.r_est,self.ref.origin,
            self.lamP,self.lamM,self.lamR,self.lamL,self.delta,self.tau,self.n_threads
            )
...
if __name__=='__main__':
    ...
    r0=8.0
#    ellipse=Ellipse(spacing,r0)
    
    

    def integrand(freq,ellipse):    
        def f(theta,phi):
            angle=(theta,phi)
            return ellipse.calc_radius(angle)*Y(freq,angle)*np.sin(angle[-1])
#            return Y((2,3),angle)*Y(freq,angle)*np.sin(angle[-1])
...
#    freq=(1,2)
    ...
    def get_coeff(freq):
        return dblquad(integrand(freq,Ellipse(spacing,r0)), 0.0,np.pi, lambda x:0,lambda x: 2*np.pi)
    
    Freq=get_spectrum(5)[1]
    coeff=[get_coeff(freq) for freq in Freq ]
...
#    PC=PolarCache([4,10,10],[3,1,1])#z,y,x dim order for entering in info here
