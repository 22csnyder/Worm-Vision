...
#    cube_display=VGroup('box_coords.x_len','box_coords.y_len','box_coords.z_len','box_coords.x_pos','box_coords.y_pos','box_coords.z_pos')
    ...
    def __init__(self,worm,smear=None,spacing=np.array([1,1,1]),neuron_window_shape=None):#nws in x,y,z format
        # Do not forget to call the parent's __init__
        HasTraits.__init__(self)
        
        self.segments=collections.OrderedDict()
        self.env=worm#hack--will be able to see read,write dir
        self.worm=worm

        #if clause nonfunctional -- only "else" works
        if isinstance(worm,np.ndarray):#first argument is a patch to use
            self.Patch=worm            
            ...
            self.polar_cache=PolarCache(self.patch_shape,self.spacing,smear)#x,y,z patch, but z,y,x spacing #z,y,x smear
            ...
            if spacing is not None:
                self.spacing=worm.spacing#gotten from .ini
            else:
                self.spacing=spacing
            self.neuron_window_shape=neuron_window_shape#x,y,z
            self.add_trait('spacing',Array(value=self.spacing))
            
            self.worm=worm
            self.max_time=self.worm.stack.shape[0]-1        
            self.worm.display_on_scene( self.whole_worm )  ######repair later######
            if worm.stack.ndim < 4:
            ...
            VGroup(
                        Item('small_scene',editor=SceneEditor(scene_class=MayaviScene),height=250, width=300, show_label=False),
                            VGroup(#Include('main_scene'),
                                HGroup('show_global_view',spring,'show_right_panel'),
                                HGroup( Item('neuron_name',editor=TextEditor(auto_set=False,enter_set=True,evaluate=str)) ,'previous_marker','next_marker'),
                                HGroup('backward',Item('time'),'forward',visible_when='has_time_component==True'),
