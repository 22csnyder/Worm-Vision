...
#pragma omp parallel reduction(+:interior_intensity)
{
	...
	for(int k=0;k<ref_shape[2];k++){
		   ...
		   k2>0 && k2<std_shape[2])
		{
			...
			long std_flat=k2+j2*std_shape[2]+i*std_shape[2]*std_shape[1];
	...
	#pragma omp single
	{
		...
		bdry_idx.reserve(prefix[nthreads]);
	}
...
void calc_delA(const std::vector<double>& distributedF,
		const std::vector<point>& bdry_idx,
		double* Awt,long* Awt_shape,double* Aml,
...
delA[freq]+=2.0*Aml[freq]*Wml[freq];
...
void calculate_changes(
        const std::vector<point>& bdry_idx,
        double* I,double* Awt,long* Awt_shape,double* Aml,double*Wml,
...
calc_delA(distributedF,bdry_idx,Awt,Awt_shape,Aml,Wml,lamL,delA);
...
calculate_changes(bdry_idx,I,Awt,Awt_shape,Aml,Wml,
