...
def lp_graph_with_custom(unaries, edges, edge_weights):
    ...
    #2 aux binary variables
    n_variables = n_nodes * n_states + n_edges * n_states ** 2  + 2

    # constraints: one per node,
    # and n_nodes * n_states for pairwise minus one redundant per edge
    #+1 constraint for sum aux variables =1
    n_constraints = n_nodes + n_edges * (2 * n_states - 1)  +1

    # offset to get to the edge variables in columns
    edges_offset = n_nodes * n_states
    
    #offset to get to aux variables
    aux_offset=edges_offset+ n_edges *(n_states**2)
    ...
    #aux
    data.append(1)
    I.append(row_idx)#1 after the last pairwise constraint
    J.append(aux_offset)
    data.append(1)
    I.append(row_idx)
    J.append(aux_offset+1)
    ...
    G = cvxopt.spdiag(cvxopt.matrix(-np.ones(n_variables)))#aux included
