...
#        print 'loss is called in base.py'
#        print 'within base.loss()...'
...
#            print 'inside base.loss(), y_hat is a tuple?'
            return self.continuous_loss(y, y_hat[0])
        if hasattr(self, 'class_weight'):
#            print 'loss returned: ',np.sum(self.class_weight[y] * (y != y_hat))
            return np.sum(self.class_weight[y] * (y != y_hat))
#        print 'loss returned: ',np.sum(y != y_hat)
        return np.sum(y != y_hat)

    def batch_loss(self, Y, Y_hat):
#        print 'batch_loss is called in base.py'
