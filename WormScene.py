...
from WormBox.VisClasses import Channel
...
class constant_camera_view(object):
    ...
    def __enter__(self):
        if self.cf==None:
            self.cf=mlab.gcf()
        self.orig_no_render=self.cf.scene.disable_render
        if not self.orig_no_render:
            self.cf.scene.disable_render=True
        cc=self.cf.scene.camera
    ...
    def __exit__(self,t,val,trace):
        ...
        if not self.orig_no_render:
            self.cf.scene.disable_render=False
        if t!=None:
            print t,val,trace
...
from WormBox.VisClasses import BoxCoords
class AxesBox(BoxCoords):
    def __init__(self,window_shape,spacing):
        self.spacing=spacing
        BoxCoords.__init__(self,window_shape,window_shape,must_have_odd_shape=False)
    ...
    def setup_grid(self):
        ...
        self.vertices=np.copy(self.unit_cube_vertices).astype(np.float)
...
#        self.x_vert=[self.vertices[0],self.vertices[1]]
#        self.y_vert=[self.vertices[0],self.vertices[2]]
#        self.z_vert=[self.vertices[0],self.vertices[4]]
...
#        a0,a1=self.x_vert
#        self.linex=mlab.plot3d([a0[0],a1[0]],[a0[1],a1[1]],[a0[2],a1[2]],tube_radius=1,color=(1,0,0),figure=scene.mayavi_scene)
#        b0,b1=self.y_vert
#        self.liney=mlab.plot3d([a0[0],a1[0]],[a0[1],a1[1]],[a0[2],a1[2]],tube_radius=1,color=(0,1,0),figure=scene.mayavi_scene)


    def update_vertices_with(self,length,origin):
        self.vertices=np.copy(self.unit_cube_vertices).astype(np.float)
        for d in range(3):
#            self.vertices[:,d]-=0.5
            self.vertices[:,d]*=(length[d]-1)*self.spacing[d]
#            self.vertices[:,d]*=length[d]
            self.vertices[:,d]+=(origin[d])####It's not really working, but I can't debug it right now
                                           ####There is a gap to one side of the miniview. worse with vol images
#            self.vertices[:,d]+=4
#        self.sgrid.points=np.floor(self.spacing*(self.vertices))#Hack, DEBUG, probably doens't even work
#        self.sgrid.points=np.floor(self.spacing)*(self.vertices)#Hack, DEBUG, probably doens't even work
        self.sgrid.points=(self.vertices)#Hack, DEBUG, probably doens't even work

#        _x=self.linex.mlab_source.z
#        _x*=length[0]
        ###Use "reset" when shape of source doesn't change
        ###Use "set" when it does
#        self.linex.mlab_source.reset(z=_x)

#        print 'new axis length',_x
#        self.linex.mlab_source.m_data.points=_x




#any additional class attirbutes have to be traits since this inherits from HasPrivateTraits
class WormScene(MlabSceneModel):
...
#    floor_downsample=Int(1)
#    ceil_downsample=Int(10)
#    downsample=Range(value=1,low='floor_downsample',high='ceil_downsample',enter_set=True,auto_set=False,mode='slider')
    
    downsample=Int(-1)    
    
#    downsample=Range(value=1,low=1,high=10,enter_set=True,auto_set=False,mode='slider')
    show_downsample=Bool(True)
    
#    worm=Instance(Channel,())
    
    has_time_component=Bool(False)
    is_local_view=Bool(False)
    ...
    axes=Instance(AxesBox,())
    
    def __init__(self,*args,**kwargs):
        MlabSceneModel.__init__(self,*args,**kwargs)
...
#    def _worm_changed(self):
#        self.ceil_downsample=self.worm.zps
#        self.mayavi_scene.scene.show_axes=True#doesn't work
#    def _name_changed(self):
######couldn't get this to work :( (no handlers found for logger mayavi.core.common
#        mlab.text(.05,.05,self.name,figure=self.mayavi_scene)#refere to scratch2.py for syntax
#        mlab.text(.05,.05,'sample',figure=self.mayavi_scene)
#    def _downsample_changed(self):
#        self.show_downsample=True #If anyone tries to access this, show it in the display

#    def _drawn_changed(self,old,new):#blarhgh  #no module to attach to for some reason
#        if new is True:
#            mlab.text(0.02,0.95,self.name,figure=self.mayavi_scene)

    def draw_axes(self,initial_shape):
        if hasattr(self,'worm'):
            spacing=self.worm.spacing
        else:
            spacing=np.array([1.0,1.0,1.0])
        self.axes=AxesBox(initial_shape,spacing)
        self.axes.display_grid_on_scene(self)
        if hasattr(self,'worm'):
            self.update_axes()
#        self.on_trait_change('worm.length.[x,y,z]',self.update_axes)#I can never get these to work
        
    def update_axes(self):
        if hasattr(self,'axes'):
            self.axes.update_vertices_with(self.worm.length,self.worm.origin)    
...
#            self._toggle_mesh_visibility_fired()
    ...
    def _is_local_view_changed(self,old,new):
        if old is False and new is True:
            self.show_downsample=False

    @on_trait_change('activated')
    def scene_activated(self):
        print 'scene is activated' 
        
#        self.camera=self.scene.camera#doesn't work#new attributes have to be declared as traits
#        iren=self.scene.interactor
#        self.pointpicker=self.scene.picker.pointpicker
#        mvt_picker = MvtPicker(self.mayavi_scene.scene,self.scene.picker.pointpicker)
#        self.scene.interactor.add_observer('RightButtonPressEvent',mvt_picker.on_button_press)
        self.scene.interactor.add_observer('RightButtonPressEvent',self.on_right_button_press)
#        self.scene.interactor.add_observer('LeftButtonPressEvent',self.on_left_button_press)###was going to make a double-click
        
#        self.scene.interactor.add_observer('RightButtonPressEvent',self.picker_callback)
        self.scene.picker.pointpicker.add_observer('EndPickEvent', self.picker_callback)  
        
        self.scene.interactor.add_observer("KeyPressEvent",self.cbk)
...
#                                  z-0.1, z+0.1)
    ...
    def on_right_button_press(self, obj, evt):
        ...
        x, y = obj.GetEventPosition()
        self.scene.picker.pointpicker.pick((x, y, 0), self.mayavi_scene.scene.renderer)
    
        print'on_right_button_press'
        print '    scenename',self.name
        print '    position:',x,' ',y
...
#            x, y = obj.GetEventPosition()
#            self.volume.pickable=0
#            self.scene.picker.pointpicker.pick((x, y, 0), self.scene.mayavi_scene.scene.renderer)
...
#        print picker_obj
        ...
        if picker_obj.view_prop is None:
...
#        print
        ...
        q1=np.array(picker_obj.mapper_position)
        q0=np.array(self.scene.camera.position)
...
#        self.sgrid=tvtk.UnstructuredGrid(points=[a0,a1])        
#        self.mayavi_scene.add_child(
...
#        self.grid_src=VTKDataSource(data=self.sgrid)
        ...
        with constant_camera_view(self.mayavi_scene):
            mlab.plot3d([a0[0],a1[0]],[a0[1],a1[1]],[a0[2],a1[2]],tube_radius=.5,name=vfname,color=(1,0,0),figure=self.mayavi_scene)

        self.scene.camera.position=np.array(self.scene.camera.position)+1#small offset so not looking directly down barrel
#        mlab.move(3,1,-1.2,figure=self.mayavi_scene)
...
#            self.line1=np.array([d,q1]).transpose()
            ...
            self.new_point=np.round(0.5*(  self.p0+alp*self.m0  +  self.p1+bet*self.m1  ))
            print 'WormScene312: new point is ',self.new_point
            
            with constant_camera_view(self.mayavi_scene):
                for child in vflines:
                    self.scene.mayavi_scene.remove_child(child)

            if hasattr(self,'worm'):#part of larger multichannel display
                self.worm.add_neuron(self.new_point,add_offset=self.is_local_view)
            else:#standalone
                point=self.new_point
                with constant_camera_view(self.mayavi_scene):
                    nodes=mlab.points3d(point[0],point[1],point[2],name='new marker',color=(1,0,0),figure=self.mayavi_scene)            
                    nodes.glyph.glyph.scale_factor=4
...
#        print self.a
    ...
    traits_view = View(
        VGroup(
#            Item('scene',editor=SceneEditor(scene_class=WormScene),height=250, width=300, show_label=False,style='custom'),
...
#            Item('scene',editor=SceneEditor(scene_class=MayaviScene),height=250, width=300, show_label=False,style='custom'),
#            Item('object.mayavi_scene',editor=SceneEditor(scene_class=MayaviScene),height=250, width=300, show_label=False,style='custom'),
#            Include('mayavi_scene'
#            Item('object',editor=SceneEditor(scene_class=MayaviScene),height=250, width=300, show_label=False,style='custom'),
#            Item('scene',height=250, width=300, show_label=False,style='custom'),#empty
#            HGroup(Item('downsample'),visible_when='show_downsample==True')
#            Item('downsample', show_label=False,visible_when='downsample!=-1')
            
#            Item('downsample',show_label=False)
#            Item('downsample',visible_when='show_downsample==True')

            
#            Item('downsample',visible_when='show_downsample==True')
            
#            Item('debug_string',show_label=False,editor=TextEditor(auto_set=False,enter_set=True,evaluate=str))
#            visible_when='downsample != -1'
            ),
#            kind='panel',
    )
...
class Worm4DScene(WormScene):
    
    max_time_step=Int(1)
    min_time_step=Int(0)
    time_step=Range(low='min_time_step',high='max_time_step',value=0,mode='slider',auto_set=False)
...
#            Include('default_view'),
            Item('name',style='readonly',show_label=False),
            Item('scene',editor=SceneEditor(scene_class=MayaviScene),height=250, width=300, show_label=False),
            Item('time_step',show_label=False),
#            Item('time_step',show_label=False,editor=RangeEditor(mode='slider')),
#            Item('time_step',show_label=False,editor=RangeEditor()),
...
if __name__=='__main__':
    ...
    I=I[:,10:65,:]
    length=np.array(I.shape,dtype=np.float)
    radius=length/2.0

    wormscene=WormScene()
    
    a=ArraySource(origin=-radius)
    a.scalar_data=I
    
    wormscene.mayavi_scene.add_child(a)
    a.add_module(Volume())


    wormscene.draw_axes(I.shape)
    wormscene.axes.update_vertices_with(length,radius)

    wormscene.axes.update_vertices_with(length,-radius) 

    wormscene.configure_traits()
